...
export interface EarthquakeHazardFeature {
  title: string;
  description: string;
}

export interface EarthquakeHazardEffect {
  label: string;
  description: string;
}

export interface EarthquakeHazardProtocol {
  title: string;
  description: string;
  highlight: string;
}

export interface EarthquakeHazardImage {
  src: string;
  alt: string;
  caption: string;
  sourceUrl?: string;
...
export interface EarthquakeHazardDetailContent {
  title: string;
  description: string;
  effects: EarthquakeHazardEffect[];
  images: EarthquakeHazardImage[];
}

interface EarthquakeHazardDetailProps {
  content: EarthquakeHazardDetailContent;
}

export const EarthquakeHazardDetail: React.FC<EarthquakeHazardDetailProps> = ({
  content,
}) => {
          ...
          <div className="pt-2 pb-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {content.effects.map((effect, idx) => (
                <div
                  key={idx}
                  className={`flex flex-col p-4 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 hover:border-primary/30 transition-all duration-300 h-full ${
                    idx === 2 ? "sm:col-span-2" : ""
                  }`}
                >
                  <strong className="text-white text-base md:text-lg mb-2 font-black tracking-wide">
                    {effect.label}
                  </strong>
