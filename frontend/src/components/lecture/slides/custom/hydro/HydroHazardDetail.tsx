...
export interface HydroHazardFeature {
  title: string;
  description: string;
}

export interface HydroHazardEffect {
  label: string;
  description: string;
}

export interface HydroHazardProtocol {
  title: string;
  description: string;
  highlight: string;
}

export interface HydroHazardImage {
  src: string;
  alt: string;
  caption: string;
  sourceUrl?: string;
...
export interface HydroHazardDetailContent {
  title: string;
  description: string;
  features?: HydroHazardFeature[];
  effects?: HydroHazardEffect[];
  protocol?: HydroHazardProtocol;
  images: HydroHazardImage[];
}

interface HydroHazardDetailProps {
  content: HydroHazardDetailContent;
}

export const HydroHazardDetail: React.FC<HydroHazardDetailProps> = ({
  content,
}) => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-full min-h-0 p-4 lg:pb-0 relative">
      ...
      <div className="flex flex-col justify-start lg:overflow-y-auto custom-scrollbar lg:pr-4 h-full">
        <div>
        ...
        {content.features && content.features.length > 0 && (
          <div className="pt-2">
            <h4 className="text-lg font-bold text-primary mb-4 uppercase tracking-wide flex items-center gap-2">
              <Info className="w-5 h-5 text-primary" /> Warning Signs
            </h4>
        ...
        {content.effects && content.effects.length > 0 && (
          <div>
            <h4 className="text-lg font-bold text-primary mb-4 uppercase tracking-wide flex items-center gap-2">
              <AlertTriangle className="w-5 h-5 text-primary" /> Effects
              Analysis
            </h4>
            <div className="flex flex-col gap-3">
        ...
        {content.protocol && (
          <div className="pt-2 pb-4">
            <h4 className="text-lg font-bold text-emerald-400 mb-4 uppercase tracking-wide flex items-center gap-2">
              <ShieldAlert className="w-5 h-5 text-emerald-400" /> Safety
              Protocol
            </h4>
            <div className="flex flex-col p-5 bg-emerald-500/10 border border-emerald-500/30 rounded-xl">
              <strong className="text-emerald-300 text-base md:text-lg mb-2 font-black tracking-wide uppercase">
